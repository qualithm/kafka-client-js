...
| `client/admin.ts`                           | `KafkaAdmin` class with topic/partition management, config describe/alter/incremental-alter, topic listing via Metadata, group management (describe/list/delete), record deletion, elect leaders, partition reassignment (alter/list), retry with exponential backoff, `createAdmin` factory                                           |
...
| `src/protocol/`      | Kafka API request/response codecs (28 APIs)                       |
